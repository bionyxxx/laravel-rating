...
class LaravelRating
{
    ...
    public function rate($user, $rateable, $value, $testimonial, $type)
    {
        if ($this->isRated($user, $rateable, $type)) {
            return $user->{$this->resolveTypeRelation($type)}()
                ...
                ->update(['value' => $value, 'testimonial' => $testimonial]);
        }
    ...
    public function getTestimonialValue($user, $rateable, $type)
    {
        $rating = $user->{$this->resolveTypeRelation($type)}()
            ->where('rateable_id', $rateable->id)
            ->where('rateable_type', $this->getRateableByClass($rateable))
        ...
        return $rating != null ? $rating->testimonial : null;
    }
